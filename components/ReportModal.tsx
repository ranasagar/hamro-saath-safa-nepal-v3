                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        <div>
                            ...
                            <select id="category" value={category} onChange={(e) => setCategory(e.target.value as IssueCategory)} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-green focus:border-brand-green text-gray-900">
                         ...
                         <div>
                            ...
                            <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-green focus:border-brand-green text-gray-900" placeholder="e.g., Dustbin near the corner is overflowing." required></textarea>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Upload Photo (Before)</label>
                            <div onClick={triggerFileSelect} className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-brand-green">
                                {photoPreview ? (
                            ...
                            <input id="file-upload" ref={fileInputRef} name="file-upload" type="file" accept="image/png, image/jpeg" className="sr-only" onChange={handleFileChange} />
                            <div className="mt-2 space-x-2 text-center">
                                {photoPreview && !isEnhanced && (
                                    <button type="button" onClick={handleEnhance} className="text-sm font-semibold bg-indigo-100 text-indigo-700 px-3 py-1 rounded-md hover:bg-indigo-200">
                                        ✨ Enhance Image
                                    </button>
                                )}
                                {photoPreview && isEnhanced && (
                                    <button type="button" onClick={handleRevert} className="text-sm font-semibold bg-gray-200 text-gray-700 px-3 py-1 rounded-md hover:bg-gray-300">
                                        Revert
                                    </button>
                                )}
                            </div>
                    ...
                    <div className="flex justify-end space-x-3 p-4 bg-gray-50 rounded-b-lg">
                        ...
                        <button type="submit" disabled={!photo || isSubmitting} className="px-4 py-2 bg-brand-green text-white rounded-md hover:bg-brand-green-dark disabled:bg-gray-400 disabled:cursor-not-allowed">
